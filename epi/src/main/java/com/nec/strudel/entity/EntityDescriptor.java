...
public class EntityDescriptor {
    ...
    public EntityDescriptor(Class<?> entityClass) {
        ...
        this.gkFinder = groupKeyFinderFor(info);
        this.keySetter = keySetterFor(info);
        this.indexes = createIndex(info);
        this.hasGeneratedGroupKey = findGeneratedGroupKeyIn(info);
        /**
         * TODO refactor: this depends on the above final fields initialized.
         */
    ...
    /**
     * Gets a list of property names: (1) when Column is specified at fields,
     * they are chosen. (2) if no Column exists, all the fields are chosen.
     * 
    ...
    private KeyGeneratorType createKeyGeneratorType(EntityInfo info) {
        ...
        /**
         * NOTE if the auto index is in-group but there are external indexes,
         * the auto index cannot be used as a counter because the external
         * indexes need a generated value that is committed. In such a case, use
         * an external counter for key generation. Then, auto index is no longer
         * "auto" so change it to a regular index.
         */
        ...
        /**
         * TODO FIXME if the key needs generated but there is no auto index,
         * generate a counter
         */
        return null;
    }
    ...
    static boolean findGeneratedGroupKeyIn(EntityInfo info) {
        for (String gk : info.groupKey()) {
            Field field = info.findField(gk);
            if (field.isAnnotationPresent(GeneratedValue.class)) {
                return true;
            }
        }
    ...
    static KeyFinder groupKeyFinderFor(EntityInfo info) {
        List<String> ids = info.key();
    ...
    static List<String> fieldsFor(EntityInfo info, On index) {
        ...
        BeanInfo keyInfo = new BeanInfo(method.getReturnType());
