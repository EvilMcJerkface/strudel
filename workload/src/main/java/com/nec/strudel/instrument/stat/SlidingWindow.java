...
public abstract class SlidingWindow<A> {
    ...
    private synchronized void tick(long time) {
        if (time > lastTick + intervalMs) {
            ...
            if (past.length < window) {
                Window<A>[] buffs = Arrays.copyOf(past, past.length + 1);
    ...
    public double averageValue(ValueCounter<A> counter) {
        Window<A>[] buffs = getPast();
        long count = 0;
        long time = 0;
        for (Window<A> w : buffs) {
            A bucket = w.get();
            count += counter.count(bucket);
            time += counter.value(bucket);
        }
        if (count == 0) {
            return Double.NaN;
        } else {
    ...
    public double countPerSec(Counter<A> counter) {
        Window<A>[] buffs = getPast();
        if (buffs.length == 0) {
            return Double.NaN;
        }
