...
/**
 * Data object that encapsulates metric values (JsonObject) and warning messages
...
public class Report {
     ...
     * 
     * @return a JsonObject that contains values and warns.
     */
    public JsonObject toJson() {
        ...
        if (!warns.isEmpty()) {
            ...
            builder.add(VALUE_WARNS, arrayBuilder);
        }
     ...
     * 
     * @return JsonObject that contains measured values
     */
    ...
    public static Report toReport(JsonObject json) {
        ...
        JsonObject counters = json.getJsonObject(VALUE_COUNTERS);
        if (counters == null) {
            counters = EMPTY_VALUE;
        }
        ...
        return new Report(counters, warns);
    }

    /**
     * Combines multiple reports into one by:
     * <ul>
     * <li>aggregating Count
     * <li>concatenating lists of warnings.
     * </ul>
     * This is used to combine reports from multiple work threads / work nodes.
     * 
     * @param reports
     */
    ...
    private static final JsonObject EMPTY_VALUE = Json.createObjectBuilder()
            .build();
