...
public class PopulateTask extends Task {
    ...
    public List<PopulateWorkItem> getWorkItems() {
        List<PopulateWorkItem> result = new ArrayList<PopulateWorkItem>();
        Random rand = getRandom();
        for (PopulateWorkItem item : process) {
            PopulateWorkItem task = createPopulateWorkItem(item, rand);
            result.add(task);
    ...
    public void addItem(PopulateWorkItem item) {
        PopulateWorkItem[] newProcess = Arrays.copyOf(process,
                process.length + 1);
