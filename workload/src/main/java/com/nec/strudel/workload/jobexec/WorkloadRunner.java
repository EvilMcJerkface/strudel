...
public class WorkloadRunner implements Runnable {
    private static final Logger LOGGER = Logger.getLogger(WorkloadRunner.class);
    ...
    private final WorkloadProfile wp = new WorkloadProfile();
    ...
    public void run() {
        ...
        try {
            ...
            LOGGER.error("workload interrupted ", ex);
        } finally {
            try {
                ctxt.terminate();
            } catch (InterruptedException ex) {
                LOGGER.error(
                        "workload shutdown interrupted ", ex);
            }
            ctxt.close();
        }
    }

    private WorkloadResult completeWork(WorkExecContext ctxt, Command com)
            throws InterruptedException {
        CommandResult commandRes = com.run(ctxt);
        if (!commandRes.isSuccessful()) {
    ...
    private WorkGroup createWorkGroup() {
        ...
        for (int i = 0; i < size; i++) {
            works[i] = new WorkRequest(nodes[i], workloadTask, dbConf);
        }
