...
public class EventExecutor<R> {
    ...
    private final BlockingQueue<R> resultQueue = new LinkedBlockingQueue<R>();
    ...
    /**
     * Stops event execution. It does not terminate events that are being
     * executed. Use awaitTermination() to make sure everything is done.
     */
    public void stop() {
        scheduler.stop();
        creator.stop();
    ...
    private static class EventScheduler<R> implements Runnable {
        ...
        private final PriorityBlockingQueue<TimedEvent<R>> queue =
                new PriorityBlockingQueue<TimedEvent<R>>(
                INIT_CAPACITY, EVENT_CMP);
        ...
        public void run() {
            try {
                ...
                LOGGER.error("event scheduler interrupted", ex);
            }
        }

        void schedule() throws InterruptedException {
            while (running) {
                TimedEvent<R> event = queue.peek();
                if (event != null) {
            ...
            exec.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        hasFailure = true;
                        LOGGER.error("event execution failed", ex);
                    }
    ...
    /**
     * Receives the results of finished events and generates next events based
     * on the given event sequence. Generated events are given to the scheduler.
     * 
     * @param <R>
     */
    private static class EventCreator<R> implements Runnable {
        ...
        public void run() {
            try {
                ...
                running = false;
                Thread.currentThread().interrupt();
                LOGGER.error("event creator interrupted", ex);
            }
        }
    ...
    static final Comparator<TimedEvent> EVENT_CMP = new Comparator<TimedEvent>() {
        @Override
