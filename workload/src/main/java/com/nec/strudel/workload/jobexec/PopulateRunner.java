...
public class PopulateRunner implements Runnable {
    private static final Logger LOGGER = Logger.getLogger(PopulateRunner.class);
    ...
    public static PopulateRunner create(PopulateTask pop, Job job) {
        ...
        /**
         * NOTE DatabaseConfig needs to know the class path to locate metadada
         * (e.g., persistence.xml for JPA) in the workload package.
         */
    ...
    private static PopulateExec createExec(PopulateTask pop,
            DatabaseConfig dbConf, Job job) {
    ...
    static class ExecImpl implements PopulateExec {
        ...
        public void execute(PopulateWorkItem item) {
            ...
            try {
                ...
                throw new WorkloadException("interrupted", ex);
            } finally {
                wg.close();
            }
        }
        ...
        void waitCompletion(PopulateWorkItem item,
            ...
            for (int i = 0; i < states.size(); i++) {
                ...
                if (BatchWorkState.isError(state)) {
                    Node node = nodes[i];
