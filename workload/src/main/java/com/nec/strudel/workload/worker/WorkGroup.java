...
public class WorkGroup implements Caller {
	private static final AtomicLong LOCAL_ID_COUNTER =
			new AtomicLong();
	protected static String newWorkId() {
		return "local." + Long.toHexString(
				LOCAL_ID_COUNTER.getAndIncrement());
	}
	public static WorkGroup create(WorkRequest... workXMLs) {
		ExecutorService exec = Executors.newCachedThreadPool();
		List<Future<InitResult>> res =
				new ArrayList<Future<InitResult>>();
		for (WorkRequest w : workXMLs) {
			res.add(exec.submit(new Initialize(w)));
		}
		Worker[] workers = new Worker[workXMLs.length];
		for (int i = 0; i < workXMLs.length; i++) {
			Future<InitResult> r = res.get(i);
			try {
				InitResult initRes = r.get();
				workers[i] = initRes.getWorker();
			} catch (InterruptedException e) {
				LOGGER.error("initialization interrupted", e);
				throw new WorkloadException(
					"initialization interrupted", e);
	...
	private static final Logger LOGGER =
		    Logger.getLogger(WorkGroup.class);
	...
	public void operate(String name, JsonObject arg)
		...
		List<Future<CommandResult>> res =
				new ArrayList<Future<CommandResult>>();
	...
	public Report getReport()
			throws InterruptedException {
		List<Future<Report>> res =
				new ArrayList<Future<Report>>();
	    for (Worker w : workers) {
	    	res.add(exec.submit(
	    			new GetReport(w)));
	    }
	...
	static class Initialize implements Callable<InitResult> {
		...
		public Initialize(WorkRequest workXML) {
			this.work = workXML;
		}
		@Override
