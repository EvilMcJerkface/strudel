...
/**
 * Tries to buy the current auction item of interest, which is specified by
 * AUCTION_ITEM_ID. The trial fails if the auction item has been sold
 * (AUCTION_SOLD) or expired (AUCTION_EXPIRED).
 *
 */
public class StoreAuctionBuyNow extends AbstractStoreAuctionBuyNow<EntityDB>
    ...
    protected EntityTask<Result> update(final BuyNowAuction bna,
            final ResultBuilder res) {
        return new EntityTask<Result>() {
            @Override
            public Result run(EntityTransaction tx) {
                AuctionItem item = tx.get(AuctionItem.class, bna.getItemId());
                Result execResult = check(bna, item, res);
