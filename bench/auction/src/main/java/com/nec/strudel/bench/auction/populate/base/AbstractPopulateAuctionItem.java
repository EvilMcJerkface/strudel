...
public abstract class AbstractPopulateAuctionItem<T>
    ...
    public boolean validate(T db, AuctionParamSet param,
        ...
        for (int i = 0; i < ps.length; i++) {
            boolean result = validate(db, ps[i], items.get(i), reporter);
            success = result && success;
        }
        return success;
    }

    private boolean validate(T db, AuctionParam param, AuctionItem item,
            ValidateReporter reporter) {
        boolean success = true;
        AuctionItem expected = new AuctionItem(item.getSellerId(),
                item.getItemNo());
        param.build(expected);
        ...
        BuyNowAuction expectedBna = param.bnaIfExists(expected);
        if (expectedBna != null) {
        ...
        if (param.numOfBids() != bids.size()) {
            reporter.error(
                    "get by auction_bid_index size=" + bids.size()
                            + " expected=" + param.numOfBids()
    ...
    /**
     * NOTE: this assumes that the automatically generated itemNo values are in
