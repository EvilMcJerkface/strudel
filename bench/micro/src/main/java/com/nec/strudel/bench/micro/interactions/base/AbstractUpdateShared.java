...
public abstract class AbstractUpdateShared<T> implements Interaction<T> {
    ...
    public void prepare(ParamBuilder paramBuilder) {
        List<Shared> itemList = paramBuilder.getList(TransitionParam.SHARED);
        if (!itemList.isEmpty()) {
            List<SharedId> idList = new ArrayList<SharedId>(
                    itemList.size());
            ...
            List<SharedId> idList = new ArrayList<SharedId>(ids.size());
