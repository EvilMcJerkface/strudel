...
public class JpaDatabaseCreator extends DatabaseCreator {
    public static final String PROP_SCHEMA_ACTION = "strudel.jpa.schema-generation.database.action";
    ...
    public JpaDatabaseCreator(TargetConfig dbConfig) {
         ...
         * javax.persistence.* since the dbConfig is shared by the workers
         * (where we do not want to let them run schema actions). So we use
         * another property to let the database creator know the option.
         */
        String action = props.getProperty(PROP_SCHEMA_ACTION,
                "drop-and-create");
